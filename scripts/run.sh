...
TOTAL_CLIENT_MACHINES=${TOTAL_CLIENT_MACHINES:-}
NODE_ID=${NODE_ID:-}
...
if [ -z "$NODE_ID" ]; then
    ...
    # Servers: .1=0, .2=1, .3=2  Clients: .4=3, .5=4, ..., .11=10
...
[ -n "$TOTAL_CLIENT_MACHINES" ] && ENV="$ENV TOTAL_CLIENT_MACHINES=$TOTAL_CLIENT_MACHINES"
